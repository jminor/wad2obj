...
__doc__ = """
Extracts textures and map geometry from a WAD file into a USDZ file
suitable for use in applications supporting Universal Scene Description.
"""
...
def usdmap(wad, name: str, filename: str, textureNames: List[str], textureSizes: Dict[str, Tuple[int, int]], centerVerts: bool) -> None:
    ...
    # adjust indices to 0-based for USD
    for poly in polys:
        poly.faces = [[i-1 for i in face] for face in poly.faces]
    ...
    # add materials
    for tex_name in textureNames:
        material_path = f'/materials/{tex_name}'
    ...
    mesh_index = 0
    for poly in polys:
        if not poly.texture or poly.texture == '-' or poly.texture == 'F_SKY1':
            continue

        mesh_path = f'/world/mesh{mesh_index}'
        mesh = UsdGeom.Mesh.Define(stage, mesh_path)

        faceVertexIndices = Vt.IntArray([i for face in poly.faces for i in face])
        faceVertexCounts = Vt.IntArray([len(face) for face in poly.faces])
        ...
        # UV coordinates
        uvs = Vt.Vec2fArray([Gf.Vec2f(u, v) for face_uvs in poly.textureCoords for u, v in face_uvs])
        primvar = UsdGeom.PrimvarsAPI(mesh).CreatePrimvar('st', Sdf.ValueTypeNames.TexCoord2fArray)
        primvar.Set(uvs)
        primvar.SetInterpolation('faceVarying')

        # bind material
        mat_prim = stage.GetPrimAtPath(f'/materials/{poly.texture}')
        if mat_prim:
    ...
    # export to USDZ
    stage.Export("temp.usd")
    UsdUtils.CreateNewUsdzPackage(Sdf.AssetPath("./temp.usd"), filename)
    os.remove("temp.usd")
...
def write_textures(wad) -> Tuple[List[str], Dict[str, Tuple[int, int]]]:
    names = []
    textureSizes = {}
...
def parse_args() -> argparse.Namespace:
    ...
    epilog = "Example: wad2usd.py doom.wad -m 'E1*' -o /tmp"
...
def main() -> None:
    ...
    textureNames, textureSizes = write_textures(inwad)
    ...
    if len(maps) == 0:
        ...
        for name in maps:
            usdfile = f"{name}.usdz"
            print(f"Writing {usdfile}")
            usdmap(inwad, name, usdfile, textureNames, textureSizes, args.center)
